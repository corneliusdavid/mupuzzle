...
type
  ...
  const
    CARAT_STR = '>';
  private
    MUString: MUStringClass := new MUStringClass;
    fSearchStr: String;
    fSelected: Boolean;
    fCanceled: Boolean;
    ...
    method ShowChangedString(NewMUString: MUStringClass);
    method RememberStringsUsed(NewMUString: MUStringClass);
  ...
  StringChangeEvent = delegate(aMUString: MUStringClass);
...
begin
  ...
  //
  Debug.WriteLine('-------------');
  lblCurrStr.Text := MUString.TheString;
  MUString.OnStringChanged += ShowChangedString;
  MUString.OnStringChanged += RememberStringsUsed;
  MUString.OnChooseSubString += ChooseSubStr;
  RememberStringsUsed(MUString);
end;
...
method frmMUMain.ShowChangedString(NewMUString: MUStringClass); 
begin
  lblCurrStr.Text := NewMUString.TheString;
end;

method frmMUMain.RememberStringsUsed(NewMUString: MUStringClass); 
begin
...
begin
  MUString.TheString := lbMyMUStrings.SelectedItem.ToString;
end;

{ MUStringClass }

method MUStringClass.AddU; 
begin
  if TheString[TheString.Length - 1] = 'I' then
    TheString := TheString + 'U';
end;

method MUStringClass.AppendAll; 
begin
  TheString := TheString + TheString.SubString(1);
end;

method MUStringClass.ReplaceIIIwithU; 
const
  III_STR = 'III';
var
  III_Index: Integer;
...
method MUStringClass.SetString(value: string); 
begin
...
method MUStringClass.DoStringChangedEvent;
begin
